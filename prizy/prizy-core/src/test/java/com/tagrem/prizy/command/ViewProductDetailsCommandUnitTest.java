...
public class ViewProductDetailsCommandUnitTest {
    ...
    @Test
    public void itShouldFetchProductDetails() {
        // given
        given(productService.findOne(anyString())).willReturn(p);
        // when
        ViewProductDetailsCommand command = factory.newViewProductDetailsCommand(productService, p.getId());
        command.execute();
        // then
        verify(productService, times(1)).findOne(p.getId());
    }
